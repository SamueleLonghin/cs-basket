...
?>
<a href="img/tretiri.pdf" target="_new"> Leggi la documentazione</a>
...
foreach ($squadre['F'] as $s) {
    if (is_array($s['Tiri3Shot'])&&count($s['Tiri3Shot'])>1) {
        stampa($s);
    }}
?>
<h2>Maschili</h2>
...
function stampa($p)
{
    ...
    <div class="col-xs-12 contenitore-coso-palle" style="display:flex;flex-wrap:wrap;min-height:100px">
        ...
        <div class="col-xs-12 col-md-6"
             style="align-self:center;display:flex;flex-wrap:nowrap;margin-top:5px;margin-bottom:5px;">
            <?php
            foreach ((array)$p['Tiri3Shot'] as $k => $t) {
                    ?>
                    <div style="min-width:30px;width:8%; padding:1px">
                        <img class="palla"
                             style="border-radius: 50%; background-color:<?= $t == "1" ? "green;" : ($t == "0" ? "red;" : ";") ?>; width:100%"
        ...
        <div class="col-xs-12 col-md-2 contenitore-minuti-punti" style="align-self:center">
            <div><strong><?= $p['Tempo3Shot'] ?></strong> secondi</div>
            <div><strong><?= $p['Punti3'] ?></strong> punti</div>
        </div>
